---
name: unity-environment-art
description: Art-direct beautiful, professional 3D environments in Unity via the unity-mcp bridge. Use when designing, building, lighting, or polishing a Unity scene/level/environment — greyboxing, lighting passes, fog/atmosphere, post-processing, materials, composition. Pairs with the unity-dev MCP (planning/metadata). Triggers: "build/design the scene", "set up lighting", "make it look atmospheric", "art direction", "environment design".
---

# Unity Environment Art Direction

Playbook cho vai trò **level artist / environment designer** dùng
[Coplay unity-mcp](https://github.com/CoplayDev/unity-mcp) để dựng môi trường 3D
đẹp và chuyên nghiệp trong Unity. Kết hợp với **unity-dev** MCP (kế hoạch/metadata).

## Nguyên tắc tối thượng: LOOK → CRITIQUE → ADJUST

**KHÔNG BAO GIỜ dựng mù bằng tọa độ.** Sau mỗi thay đổi đáng kể:
1. Chụp screenshot scene/game view (unity-mcp).
...
## Thứ tự dựng (đúng quy trình pro)
...
- **Color temperature**: lạnh (xanh) cho cô đơn/đáng sợ, ấm (cam) cho an toàn.
- Contrast sáng-tối tạo chiều sâu (chiaroscuro). Vùng tối quan trọng như vùng sáng.
...
- **Fog** tạo atmospheric perspective (chiều sâu) + giấu giới hạn scene + mood.
  Không gian vũ trụ/kinh dị: fog mật độ vừa, màu tối lạnh.
...
Trước khi dựng một scene, xác định reference (phim, game, ảnh) cho mood.
...
- Dựng xong 1 pass: `update_scene status=in_progress`, cập nhật `assets` đã dùng.
- Hoàn thiện scene: `update_scene status=done`.

## Recipe: game vũ trụ cô độc (vd "The Last Signal")
...
- Ánh sáng cabin: key light ấm yếu (an toàn giả tạo) tương phản cái lạnh bên ngoài.
