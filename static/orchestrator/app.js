...
// Đổi workspace đang lọc → refetch sessions/signals theo scope đó (xem lại từ trang đầu).
function onWsFilter() {
  currentWS = $("ws-filter").value;
  sigShown = runsShown = PAGE;   // đổi tenant → reset phân trang
  refreshAll();
}
window.onWsFilter = onWsFilter;
...
// Populate dropdown lọc (header) + dropdown workspace ở form spawn, giữ lựa chọn hiện tại.
function renderWorkspaces(list) {
  WORKSPACES = list;
  const opt = (w) => `<option value="${esc(w.id)}">${esc(w.name || w.id)}${w.status !== "active" ? " (" + w.status + ")" : ""}</option>`;
  const filt = $("ws-filter");
  filt.innerHTML = `<option value="">Tất cả (${list.length})</option>` + list.map(opt).join("");
  filt.value = currentWS;
  // form spawn: bỏ 'default' khỏi danh sách tenant riêng? Giữ để vẫn spawn được vào default.
  const spws = $("sp-ws");
  if (spws) {
    const cur = spws.value;
    spws.innerHTML = `<option value="">default</option>` +
      list.filter((w) => w.id !== "default").map(opt).join("");
    if ([...spws.options].some((o) => o.value === cur)) spws.value = cur;
    else if (currentWS && currentWS !== "default") spws.value = currentWS;  // theo filter hiện tại
  }
  renderWsBanner();
}

// Banner dưới header: khi đang lọc 1 workspace cụ thể → hiện trạng thái + nút suspend/activate/kill.
function renderWsBanner() {
  const el = $("ws-banner");
  const w = WORKSPACES.find((x) => x.id === currentWS);
  if (!currentWS || !w) { el.classList.remove("show"); return; }
  el.classList.add("show");
  $("ws-banner-info").innerHTML =
    `<b>${esc(w.name || w.id)}</b> · <code>${esc(w.id)}</code> · ${w.sessions} session · ` +
    `${badge(w.status, w.status === "active" ? "b-green" : "b-amber")}` +
    (w.kill_switch ? " " + badge("kill switch", "b-red") : "") +
    (w.root_dir ? ` · <code>${esc(w.root_dir)}</code>` : "");
  ...
  const kill = w.kill_switch
    ? `<button onclick="act('/api/workspaces/${encodeURIComponent(w.id)}/resume')">Resume runs</button>`
    : `<button class="danger" onclick="act('/api/workspaces/${encodeURIComponent(w.id)}/stop')">Stop runs</button>`;
  $("ws-banner-actions").innerHTML = w.id === "default" ? "" : suspend + kill;
}
...
async function sendSignal() {
  ...
  try {
    const r = await api("/api/signals", "POST", {
      ...
      workspace_id: currentWS || undefined,   // resolve role trong workspace đang lọc (nếu có)
...
async function refreshAll() {
  try {
    const q = wsQuery();
    const [workspaces, sessions, signals, runs, health] = await Promise.all([
      api("/api/workspaces"),
      api("/api/sessions" + q),
    ...
    // Nếu workspace đang lọc bị xóa/không còn → về "Tất cả".
